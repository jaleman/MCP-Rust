...
use anyhow::Result;
use mcp_server::bundle::{list_docs_in, load_bundle, resource_stem_is_safe};
...
// The MCP server struct. Configuration is injected through the constructor and
// carried as a field — the environment is read exactly once, in main(). The
// tool_router field is generated by the #[tool_router] macro and holds the
// dispatch table that routes incoming tool calls to the correct method.
// #[allow(dead_code)] silences the warning about it never being read directly.
#[derive(Clone)]
struct KukaServer {
    /// Where the knowledge bundle lives. Resolved once at startup.
...
#[tool_router]
impl KukaServer {
    fn new(knowledge_dir: PathBuf) -> Self {
        Self {
            knowledge_dir,
    ...
    fn list_docs(&self) -> Result<CallToolResult, McpError> {
        list_docs_in(&self.knowledge_dir)
    }
    ...
    fn search_docs(
        ...
        Ok(run_search(&self.knowledge_dir, &input.query))
    }
}
...
impl ServerHandler for KukaServer {
    ...
    async fn list_resources(
        ...
        let docs = load_bundle(&self.knowledge_dir).map_err(|e| McpError {
...
async fn main() -> Result<()> {
    ...
    // Everything downstream receives the resolved path — nothing else in the
    // program touches the environment.
    let knowledge_dir = PathBuf::from(
        std::env::var("KUKA_KNOWLEDGE_DIR").unwrap_or_else(|_| "knowledge".to_string()),
    );

    tracing::info!(
        "Starting KUKA MCP server (knowledge dir: {})",
        knowledge_dir.display()
    );

    // Attach the server to stdin/stdout and block until the client disconnects
    let service = KukaServer::new(knowledge_dir).serve(stdio()).await?;
...
Reflectors must be mounted at a height of 150 to 2000 mm above floor level.";
    ...
    // The point of step 4: a fully-wired server can now be constructed against
    // any directory. Before, KukaServer::new() implicitly depended on the
    // process environment + working directory, so the tool methods could not
    // be exercised from a test at all.
    #[test]
    fn server_tools_use_injected_knowledge_dir() {
        let temp_dir = bundle_with_one_doc();
        let server = KukaServer::new(temp_dir.path().to_path_buf());

        let list = server.list_docs().unwrap();
        assert!(result_text(&list).contains("Reflector Guide"));
        ...
        let search = server
            ...
            .unwrap();
        assert!(result_text(&search).contains("Found 1 result(s)"));
    }
}
